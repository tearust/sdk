...
pub type InvokeLayout<T = Vec<u8>> = (UseFromToBytes<ActorId>, u64, T);
...
where
	T: ToBytes,
{
	let input = (kind as _, quote_id.map(|x| x as _), (id, budget, msg));
	let mut result =
...
pub fn decode_invoke(mut invoke: &[u8]) -> Result<(ActorId, u64, &[u8])> {
