...
use super::{wasm::NativeBudget, ActorAgent, ActorContext, ActorKind, DynFuture};
		...
		T: Actor,
	{
		...
		tokio::spawn(async move {
			...
			while let Some(msg) = {
				...
				match msg {
					Message::Invoke(msg, quote, caller, stack, billing, mut budget) => {
						let caller = caller.or_else(|| stack.last.as_deref().map(|x| x.id.clone()));
						let exec = CALLING_STACK.scope(stack, async {
							if !activated {
								budget
									.extract(Activate.price().unwrap())?
									.provide(origin.activate(caller.clone()))
									.await?;
								activated = true;
							}
							...
							budget.provide(origin.invoke(msg, caller)).await
						});
					...
					Message::Activate(quote, caller, stack, billing, budget) => {
						let caller = caller.or_else(|| stack.last.as_deref().map(|x| x.id.clone()));
						let exec = budget.provide(CALLING_STACK.scope(stack, async {
							if activated {
								return Ok(());
							}
					...
					Message::Deactivate(quote, caller, stack, billing) => {
						...
						let exec = origin.deactivate(caller);
