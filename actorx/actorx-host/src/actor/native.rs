use super::{
    calling_context, looped::ActorFactory, wasm::send_cost, Actor, ActorAgent, ActorContext,
    ActorKind,
};
...
use tea_sdk::pricing::PricedOrDefault;
    ...
    AutoTraitWrapper<Req>: NotSliceU8,
{
    ...
    > {
        if let Some(context) = calling_context() {
            if let Some(price) = req.price() {
                if let Err(e) = send_cost(&context, price).await {
