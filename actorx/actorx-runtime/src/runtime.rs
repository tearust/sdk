...
fn invoke(reg_id: &[u8], actor_id: u128, msg: &impl ToBytes, waits: bool) -> Result<InvokeActor> {
...
{
	let actor_id = actor_id.into();
	let msg = invoke(&actor_id.reg, actor_id.inst.into(), &arg, true)?.await;
...
{
	let actor_id = actor_id.into();
	let msg = invoke(&actor_id.reg, actor_id.inst.into(), &arg, false)?.await;
	let (kind, _, _, msg) = decode_input(&msg)?;
	match kind {
