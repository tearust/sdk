...
pub fn emit<const WRAP_ERROR: bool>(source: &DefineScope) -> TokenStream {
	...
	let def_summary = quote! {
		default fn summary(v: &T) -> Option<::std::borrow::Cow<str>> {
			#(if let Some(r) = <#parents as ::tea_codec::errorx::Descriptor<T>>::summary(v) {
				return Some(r);
			})*
			Some(::std::format!("{}", ::tea_codec::ImplDefault(v)).into())
		}
	};

	let def_detail = quote! {
		default fn detail(v: &T) -> Option<::std::borrow::Cow<str>> {
			#(if let Some(r) = <#parents as ::tea_codec::errorx::Descriptor<T>>::detail(v) {
				return Some(r);
			})*
			Some(::std::format!("{:?}", ::tea_codec::ImplDefault(v)).into())
		}
	...
	let impl_marks = quote! {
		#(
			impl<T> ::tea_codec::errorx::DescriptableMark<T> for #name
			where
				#parents: ::tea_codec::errorx::DescriptableMark<T>,
				T: ?Sized,
			{
			}
		)*
...
fn emit_definition(scope: &Ident, def: &TypedDefinition) -> TokenStream {
	...
	quote! {
		impl ::tea_codec::errorx::DescriptableMark<#r#type> for #scope {}
