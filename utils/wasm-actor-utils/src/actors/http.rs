    ...
    <B as Body>::Error: Into<Error> + Send + Sync + 'static,
{
    async fn request_raw(self) -> Result<Response<Bytes>> {
        ...
        let http_actor_codec::Response(result) = tea_actorx_runtime::call(
            tea_actorx_core::RegId::Static(http_actor_codec::NAME).inst(1),
            http_actor_codec::Request(uri.to_string(), data),
        )
        .await?;
